...
2. 🔄 回覆速度優化 — code complete locally on 2026-07-14, production verification pending. Weather/seeing fetch is parallelized, full `[耗時]` timing instrumentation is present, and Gemini 2.5 Flash reduced measured production `generate_reply` latency by 77%. The remaining open-exploration bottleneck was fixed with vectorized Skyfield target calculations: the 117-target local benchmark dropped from 7.746s to 0.173s, and the local `run_query()` astronomy/CCI path completed in 0.21s with external APIs replaced by fixtures. Do not re-profile or reimplement this path; the next action is deploy, run real named-target and open-exploration LINE queries, and use Render timing logs to confirm production P90 < 15s. Mark ✅ only after that live check.
...
4. ⚠️ 自定義地點別名支援 — **user explicitly flagged this as an important improvement item on 2026-07-14**, now formally logged as Phase 3B #4 in `ROADMAP.md` (table renumbered, old #4–#15 shifted to #5–#16). Not yet built, not urgent — nothing is broken today. Root cause: Chinese homophones in voice transcription (e.g. 摩/魔, both "mó") can produce a different string than what's saved for a custom (自定義) location, so a later voice query for the same place could wrongly look unrecognized. The fix is to extend the existing `aliases` mechanism (already used for the 113 approved locations) to custom Sheet-saved locations too, which currently hardcode `aliases: []` in `save_custom_location()` — not a new fuzzy-matching subsystem. Deferred because the concrete trigger case (南橫摩天) isn't even saved to the location DB yet. Its priority relative to the rest of the renumbered #5–#16 list has not been decided by the user — don't assume it's next just because it's numbered #4; ask before reordering further.
