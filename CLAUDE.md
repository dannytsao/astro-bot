...
4. ✅ 自定義地點別名與安全模糊候選 — done and live 2026-07-14. The 「自定義地點」 Sheet now has a sixth 「別名」 column; exact canonical/alias matches remain the first and directly usable path. When exact matching fails, character similarity may propose one candidate, but the Bot must wait for explicit user confirmation before applying canonical coordinates; ambiguous or rejected candidates return to the coordinate prompt. The real LINE flow `阿裏山 → 阿里山 → 是，使用這個地點 → 接續計算` passed after fixing a duplicate-normalization confirmation loop in commit `7cf187a`. Do not re-propose this as unfinished.

Production bugs found and fixed on 2026-07-14 are recorded in `CHANGELOG.md`: custom Sheet locations not reloading in a running process, the boot-time `init_sheets()` import-order `NameError`, and the safe fuzzy candidate confirmation loop caused by duplicate intent normalization. All are confirmed fixed live.
