...
| 11 | 彗星即時座標整合（第二層） | 維護可觀測彗星清單 + 接入 MPC（Minor Planet Center）自動抓取當前亮彗星；每日凌晨 2 點更新當日 + 未來 15 天座標並快取（本地或 Google Sheets）；查詢時直接讀快取，快取失效時降級為第一層（氣象 CCI）並告知用戶；完成後開放彗星方位角與目標可見性 CCI 因子 |
| 12 | 視寧度資料來源升級（Meteoblue） | 評估 Meteoblue astronomy seeing API 對台灣山區的覆蓋精度與成本；確認優於 7Timer 後替換；Windy 不採用（無直接視寧度 API） |
| 13 | 流星雨 ZHR 即時修正（IMO Live） | 接入 IMO Live ZHR，於流星雨極大期提供即時 ZHR 修正，取代靜態歷史平均值；GMN 即時通量資料格式複雜，暫不考慮 |
| 14 | 南十字座地點限制強化 | `taiwan_locations.json` 加入 `southern_horizon_clear` 欄位；南十字座查詢同時檢查緯度 < 24.5°N 且 `southern_horizon_clear: true`；不符合條件地點回覆明確提示 |
