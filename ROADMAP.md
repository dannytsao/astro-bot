...
| 3 | 語音輸入支援 | 接收 LINE 音訊訊息（AAC）→ Google Gemini API 語音轉文字 → 轉成文字後進入現有查詢流程；先以 Gemini 2.5 Flash 試行，效果不足再升 2.5 Pro；辨識失敗時回覆「沒有聽清楚，請用文字輸入」，不將低信心結果送入查詢流程；需在實作前確認 Gemini API 對 LINE AAC 格式的直接支援，或評估是否需要格式轉換。**2026-07-14 調整為優先於 #4–#11，使用者明確決定**；回覆維持現有文字格式，不做語音回覆 |
