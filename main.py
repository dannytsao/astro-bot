import math, requests, json, re, asyncio, logging
...
TARGET_LIBRARY = [
    ...
    {"name": "紫金山-ATLAS 彗星", "ra_hours": 3.20,   "dec_degrees": 15.0,   "type": "comet",         "min_alt": 10, "max_alt": 60},
]
...
def compute_target_windows(observer, target, query_dates):
    ...
    for d in query_dates:
        for minute_offset in range(0, 10 * 60, 10):
            dt_tst = datetime(d.year, d.month, d.day, 19, 0, tzinfo=timezone.utc) \
            ...
            if target.get("min_alt", 10) <= alt.degrees <= target.get("max_alt", 80):
                windows.append({
                    "target_name": target["name"], "target_type": target["type"],
                    "datetime_tst": dt_tst,
    ...
    for w in windows:
        ...
        if d not in best_per_day or w["alt_deg"] > best_per_day[d]["alt_deg"]:
            best_per_day[d] = w
    return list(best_per_day.values())
...
def get_moon_info(observer, query_dates):
    ...
    for d in query_dates:
        ...
        results.append({
            "date": d,
            "moon_phase_pct":  round(float(moon_phase.degrees) / 360.0 * 100, 1),
            "moon_phase_desc": get_moon_phase_emoji(float(moon_phase.degrees) / 360.0),
        })
    return results
...
def check_weather_multi(lat, lon, query_dates):
    ...
    valid_dates       = [d for d in query_dates if today <= d <= max_forecast_date]
    fallback          = {"cloud_cover": -1, "humidity": -1, "temp_c": -1,
    ...
    url = (f"https://api.open-meteo.com/v1/forecast"
           ...
           f"&start_date={min(valid_dates).isoformat()}&end_date={max(valid_dates).isoformat()}"
           f"&timezone=Asia%2FTaipei")
    raw = requests.get(url, timeout=10).json()
    if "hourly" not in raw:
        return {d: {**fallback, "note": "API 錯誤"} for d in query_dates}
    ...
    for d in query_dates:
        ...
        for h in [20, 21, 22, 23, 0, 1, 2]:
            check_d = d if h >= 20 else d + timedelta(days=1)
            dt_key  = datetime(check_d.year, check_d.month, check_d.day, h)
            if dt_key in hour_index:
                night.append(hour_index[dt_key])
        if night:
            avg_cloud = round(sum(x["cloud_cover"] for x in night) / len(night), 1)
            avg_hum   = round(sum(x["humidity"]    for x in night) / len(night), 1)
            avg_temp  = round(sum(x["temp_c"]       for x in night) / len(night), 1)
            avg_dew   = round(sum(x["dew_point_c"]  for x in night) / len(night), 1)
            daily[d]  = {
                "cloud_cover": avg_cloud, "humidity": avg_hum,
                "temp_c": avg_temp,       "dew_point_c": avg_dew,
                "dew_risk":     (avg_temp - avg_dew) < 3.0,
                "good_weather": avg_cloud <= 40,
            }
    return daily
...
def match_targets(target_names: list) -> list:
    if not target_names:
...
def run_query(user_query: str) -> dict:
    ...
    targets_to_check = match_targets(intent.get("targets", []))
    all_windows      = []
    ...
    good_windows = [w for w in all_windows if w.get("good_weather", False)]
    return {"intent": intent, "good_windows": good_windows[:10],
            "moon_info": moon_info, "showers": showers}
...
def generate_reply(result: dict) -> str:
    ...
    moon_summary = json.dumps([{"日期": m["date"].isoformat(),
                                 "月相": m["moon_phase_desc"]}
                                for m in moon_info], ensure_ascii=False)
    shower_text  = json.dumps([{"流星雨": s["name"],
                                 "距極大期": f"{s['days_to_peak']:+d}天",
...
總長不超過 380 字。"""
    ...
    resp = client.messages.create(
        ...
        messages=[{"role": "user", "content":
                   ...
                   f"候選時刻：\n{windows_summary if good else '無符合條件的時刻'}\n"
                   f"月相：{moon_summary}\n流星雨：{shower_text}"}]
    )
